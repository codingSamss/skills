...
3. Run `bird --cookie-source chrome whoami` to verify authentication

## Global Options

All commands should use `--cookie-source chrome` to only use Chrome cookies (skip Safari/Firefox):
```bash
bird --cookie-source chrome <command>
```
...
```bash
bird replies <tweet-id-or-url> -n 20
```
